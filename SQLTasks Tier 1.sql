...
LEFT JOIN
	Members as m
USING (memid)
WHERE DATE(b.starttime) = '2012-09-14' AND ((b.memid <>0 AND b.slots * f.membercost > 30 ) OR (b.memid =0 AND b.slots * f.guestcost > 30))
ORDER BY cost DESC
...
LEFT JOIN
	...
	FROM Members ) as m
USING (memid)
WHERE DATE(b.starttime) = '2012-09-14' ) AS tot
WHERE tot.cost > 30
...
SELECT f.name AS facility, MONTH(b.starttime) AS month, COUNT(f.name) AS count
FROM Bookings AS b
INNER JOIN Facilities AS f
USING (facid)
INNER JOIN Members AS m
